...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/healthtriage";
  ...
  try {
    ...
    for (let i = 0; i < createdPatients.length; i++) {
      ...
      await Assessment.create({
        ...
        recommendations: [
          risk.level === "HIGH"
            ? "🔴 PRIORITY: Patient should be seen immediately"
            : risk.level === "MEDIUM"
              ? "🟡 ELEVATED: Patient should be seen within 30 minutes"
    ...
    for (let i = 0; i < waitingPatients.length; i++) {
      ...
      // Randomize wait times: Base 15 mins + random jitter between -5 to +10 mins
      const randomJitter = Math.floor(Math.random() * 15) - 5; // -5 to +10
      const minutesAgo = i * 15 + randomJitter;
      ...
      await QueueEntry.create({
        ...
        status: "waiting",
        facilityId: "PHC-001",
        addedAt: new Date(Date.now() - Math.max(2, minutesAgo) * 60000), // Ensure at least 2 mins ago
      });
    ...
    seedUsers.forEach((u) => {
      console.log(
        `   ${u.role.toUpperCase().padEnd(8)} | ${u.email.padEnd(25)} | ${u.password}`,
      );
    });
    console.log("─".repeat(50));
